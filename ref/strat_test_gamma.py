...
for gamma in [0.05, 0.08, 0.1, 0.2]:
    ...
    for i in range(len(date_list)-1):
        ...
        gamma_M = gamma / np.sqrt(12)  # 年化波动率阈值  一般控制在2%  5%  8%
        ...
        # 均值 - 风险惩罚
        w = BasicMarkowitzWithRiskPenalty(n,mu,GT,xb,size,industry,lmbd,gamma_M,sizeStockLimit,industryStockLimit)
        ...
        # 可以封装函数
        next_M_price = price_day.loc[next_date, code_list].fillna(method='ffill').apply(lambda x:x/x[0])

        # prev_M_price = stock_date_M['月末收盘价']
        # price_daily = next_M_ret.copy()
        # for k in range(prev_M_price.shape[0]):
        #     price_daily.iloc[:, k] = np.cumprod(next_M_ret.iloc[:, k]) * prev_M_price[k]
        ...
        # 3、求解结果展示
        print('========gamma='+str(gamma)+'--'+date+'优化结果'+'========')
    ...
    # port_ret.to_csv('/Users/zhangchi/富国实习/组合优化/Output/中证500增强/指数内增强组合日度收益（均值模型）.csv', header=True)
    port_ret.to_csv('/Users/zhangchi/富国实习/组合优化/Output/沪深300增强/gamma='+str(gamma)+'指数内增强组合日度收益.csv', header=True)
    ...
    ax.plot(port_netVal, 'r-', label='portfolio')
    ax.plot(b_netVal, 'b-', label='benchmark')
